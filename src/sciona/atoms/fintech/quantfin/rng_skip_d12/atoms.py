...
def mulmod64_inner_step(
    ...
    """Execute one step of the shift-and-add loop for mul_mod_64.
    ...
    """
    # Shift-and-add loop for mul_mod_64.
...
def powmod64_inner_step(
    ...
    """Execute one step of the square-and-multiply loop for pow_mod_64.
    ...
    """
    # Square-and-multiply loop for pow_mod_64.
...
def _randomword32_ffi(c, state, state_prime, x, xor):
    """Wrapper that calls the Haskell version of random_word_32."""
    _lib = ctypes.CDLL("./random_word_32.so")
...
def _randomint_ffi(fromIntegral, g, g_prime, i):
    """Wrapper that calls the Haskell version of random_int."""
    _lib = ctypes.CDLL("./random_int.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
...
def _randomword64_ffi(buildWord64_prime, x, x_prime, y1, y2):
    """Wrapper that calls the Haskell version of random_word_64."""
    _lib = ctypes.CDLL("./random_word_64.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
    _func.argtypes = [ctypes.c_void_p] * 5
    _func.restype = ctypes.c_void_p
...
def _randomdouble_ffi(div, fromIntegral, val, x, x_prime):
    """Wrapper that calls the Haskell version of random_double."""
    _lib = ctypes.CDLL("./random_double.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
    _func.argtypes = [ctypes.c_void_p] * 5
    _func.restype = ctypes.c_void_p
...
def _randomint64_ffi(fromIntegral, g, g_prime, i):
    """Wrapper that calls the Haskell version of random_int_64."""
    _lib = ctypes.CDLL("./random_int_64.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
    _func.argtypes = [ctypes.c_void_p] * 4
    _func.restype = ctypes.c_void_p
...
def _addmod64_ffi(a, b, m, mod):
    """Wrapper that calls the Haskell version of add_mod_64."""
    _lib = ctypes.CDLL("./add_mod_64.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
    _func.argtypes = [ctypes.c_void_p] * 4
    _func.restype = ctypes.c_void_p
...
def _mulmod64_ffi(a, b, f, m):
    """Wrapper that calls the Haskell version of mul_mod_64."""
    _lib = ctypes.CDLL("./mul_mod_64.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
    _func.argtypes = [ctypes.c_void_p] * 4
    _func.restype = ctypes.c_void_p
...
def _powmod64_ffi(a, e, f, m):
    """Wrapper that calls the Haskell version of pow_mod_64."""
    _lib = ctypes.CDLL("./pow_mod_64.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
    _func.argtypes = [ctypes.c_void_p] * 4
    _func.restype = ctypes.c_void_p
...
def _f_mulmod_ffi(a_prime, a1, b_prime, b1, f, otherwise, r, r_prime):
    """Wrapper that calls the Haskell version of f (mul_mod_64 loop)."""
...
def _f_powmod_ffi(acc, acc_prime, e_prime, e1, f, otherwise, sqr, sqr_prime):
    """Wrapper that calls the Haskell version of f (pow_mod_64 loop)."""
    _lib = ctypes.CDLL("./f.so")
    _func_name = 'placeholder'
    _func = _lib[_func_name]
    _func.argtypes = [ctypes.c_void_p] * 8
